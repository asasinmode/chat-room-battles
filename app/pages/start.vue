...
<template>
	<section class="h-full flex flex-col overflow-x-hidden overflow-y-auto">
		<div class="flex flex-col items-center b-(b-2 black)" :class="isCreatingRoom ? 'pb-4' : ''">
			<button
				class="col-span-full h-12 w-full text-center font-600 leading-11 tracking-wider uppercase"
				:class="isCreatingRoom ? '' : 'hoverable:bg-zinc-2'"
			...
			<template v-if="isCreatingRoom">
				<div class="flex flex-wrap justify-center gap-x-5 gap-y-2 px-2">
					<span class="col-span-2 w-fit justify-self-center rounded-lg bg-zinc-2 px-2 py-1 text-black">
						<span class="text-zinc">{{ $config.public.appUrl }}/play?code=</span>{{ generatedRoomCode }}
					</span>
					<button class="button-blue-4 w-fit rounded-lg px-2 py-1 font-600 uppercase hoverable:bg-blue-5">
						copy link
					</button>
				</div>
		...
		<div class="flex flex-col items-center b-(b-2 black) pb-4">
			...
			<div class="flex gap-2 px-2">
				...
				<input
					...
					placeholder="123456"
					class="min-w-0 w-18 justify-self-center rounded-lg bg-zinc-2 px-1 py-1 text-center"
				>
				<NuxtLink
					:to="`/play?joinRoom=${roomCodeToJoin}`"
					class="button-blue-4 ml-3 w-fit self-end justify-self-start hoverable:bg-blue-5"
				>
					Join
				</NuxtLink>
			</div>
		...
		<p class="mx-2 my-4 w-fit self-center b-(2 yellow) rounded-lg bg-yellow-1 px-3 py-1 text-center text-5">
